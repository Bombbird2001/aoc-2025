...
int isFresh(vector<range> *r, int64_t value) {
	...
	while (start <= end) {
		mid = (start + end) / 2;
		if (start == end) break;
		if ((*r)[mid].start > value) {
			end = mid - 1;
		} else if (mid + 1 < (*r).size()) {
			if ((*r)[mid + 1].start <= value) {
				start = mid + 1;
			} else {
				end = mid;
			}
	...
	return (*r)[mid].start <= value && (*r)[mid].end >= value;
}
...
int main() {
	...
	int freshCount = 0;
	for (auto i: items) {
		freshCount += isFresh(&combinedRange, i);
	}
